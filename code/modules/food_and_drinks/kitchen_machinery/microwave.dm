...
/obj/machinery/oven_old/New()
	create_reagents(100)
	var/obj/item/weapon/circuitboard/machine/B = new /obj/item/weapon/circuitboard/machine/oven_old(null)
	B.apply_default_parts(src)
	..()

/obj/item/weapon/circuitboard/machine/oven_old
	name = "Oven (Machine Board)"
	build_path = /obj/machinery/oven_old
	origin_tech = "programming=1;magnets=3"
	req_components = list(
							/obj/item/weapon/stock_parts/micro_laser = 1,
							/obj/item/weapon/stock_parts/matter_bin = 1,
							/obj/item/stack/cable_coil = 2,
...
/obj/machinery/oven_old/RefreshParts()
	var/E
	var/max_items = 10
	for(var/obj/item/weapon/stock_parts/micro_laser/M in component_parts)
		E += M.rating
...
/obj/machinery/oven_old/attackby(obj/item/O, mob/user, params)
	if(operating)
		return
	if(!broken && dirty<100)
		if(default_deconstruction_screwdriver(user, "oven_old", "oven_old", O))
			return
		if(default_unfasten_wrench(user, O))
			return
		if(exchange_parts(user, O))
	...
	if(src.broken > 0)
		if(src.broken == 2 && istype(O, /obj/item/weapon/wirecutters)) // If it's broken and they're using a screwdriver
			user.visible_message( \
				"[user] starts to fix part of the oven.", \
				"<span class='notice'>You start to fix part of the oven...</span>" \
			)
			if (do_after(user,20*O.toolspeed, target = src))
				user.visible_message( \
					"[user] fixes part of the oven.", \
					"<span class='notice'>You fix part of the oven.</span>" \
				)
				src.broken = 1 // Fix it a bit
		else if(src.broken == 1 && istype(O, /obj/item/weapon/weldingtool)) // If it's broken and they're doing the wrench
			user.visible_message( \
			...
			if (do_after(user,20*O.toolspeed, target = src))
				user.visible_message( \
					"[user] fixes the oven.", \
					"<span class='notice'>You fix the oven.</span>" \
				)
				src.icon_state = "oven_old"
				src.broken = 0 // Fix it!
				src.dirty = 0 // just to be sure
				src.container_type = OPENCONTAINER
				return 0 //to use some fuel
		...
		if(clean_spray.reagents.has_reagent("cleaner",clean_spray.amount_per_transfer_from_this))
			...
			playsound(loc, 'sound/effects/spray3.ogg', 50, 1, -6)
			user.visible_message( \
				"[user] has cleaned the oven.", \
				"<span class='notice'>You clean the oven.</span>" \
			)
			src.dirty = 0 // It's clean!
			src.broken = 0 // just to be sure
			src.icon_state = "oven_old"
		...
		var/obj/item/weapon/soap/P = O
		user.visible_message( \
			"[user] starts to clean the oven.", \
			"<span class='notice'>You start to clean the oven...</span>" \
		)
		if (do_after(user, P.cleanspeed, target = src))
			user.visible_message( \
				"[user] has cleaned the oven.", \
...
/obj/machinery/oven_old/attack_paw(mob/user)
	return src.attack_hand(user)

/obj/machinery/oven_old/attack_ai(mob/user)
	return 0

/obj/machinery/oven_old/attack_hand(mob/user)
	if(..())
		return
	user.set_machine(src)
	interact(user)
...
/obj/machinery/oven_old/proc/cook()
	if(stat & (NOPOWER|BROKEN))
		return
	start()
...
/obj/machinery/oven_old/proc/microwaving(seconds as num)
	for (var/i=1 to seconds)
		if (stat & (NOPOWER|BROKEN))
			return 0
		use_power(500)
...
/obj/machinery/oven_old/proc/has_extra_item()
	for (var/obj/O in contents)
		if ( \
				!istype(O,/obj/item/weapon/reagent_containers/food) && \
				!istype(O, /obj/item/weapon/grown) \
...
/obj/machinery/oven_old/proc/start()
	visible_message("The microwave turns on.", "<span class='italics'>You hear a oven humming.</span>")
	operating = 1
	icon_state = "oven_on_old"
	updateUsrDialog()

/obj/machinery/oven_old/proc/abort()
	operating = 0 // Turn it off again aferwards
	icon_state = "oven_old"
	updateUsrDialog()

/obj/machinery/oven_old/proc/stop()
	playsound(src.loc, 'sound/machines/ding.ogg', 50, 1)
	abort()

/obj/machinery/oven_old/proc/dispose()
	for (var/obj/O in contents)
		O.forceMove(src.loc)
	to_chat(usr, "<span class='notice'>You dispose of the oven contents.</span>")
	updateUsrDialog()

/obj/machinery/oven_old/proc/muck_start()
	playsound(src.loc, 'sound/effects/splat.ogg', 50, 1) // Play a splat sound
	icon_state = "oven_old" // Make it look dirty!!

/obj/machinery/oven_old/proc/muck_finish()
	playsound(src.loc, 'sound/machines/ding.ogg', 50, 1)
	visible_message("<span class='warning'>The oven gets covered in muck!</span>")
	dirty = 100 // Make it dirty so it can't be used util cleaned
	icon_state = "oven_old" // Make it look dirty too
	operating = 0 // Turn it off again aferwards
	updateUsrDialog()
	for(var/obj/item/weapon/reagent_containers/food/snacks/S in src)
		if(prob(50))
...
/obj/machinery/oven_old/proc/broke()
	var/datum/effect_system/spark_spread/s = new
	s.set_up(2, 1, src)
	s.start()
	icon_state = "oven_old" // Make it look all busted up and shit
	visible_message("<span class='warning'>The oven breaks!</span>") //Let them know they're stupid
	broken = 2 // Make it broken so it can't be used util fixed
	flags = null //So you can't add condiments
	operating = 0 // Turn it off again aferwards
	updateUsrDialog()

/obj/machinery/oven_old/Topic(href, href_list)
	if(..() || panel_open)
		return
