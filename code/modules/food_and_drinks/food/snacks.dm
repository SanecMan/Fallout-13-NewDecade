	...
	if(iscarbon(M))
		...
		if(M == user)								//If you're eating it yourself.
			if(junkiness && M.satiety < -150 && M.nutrition > NUTRITION_LEVEL_STARVING + 50 )
				...
				to_chat(M, "<span class='notice'>¬ы жрЄте [src] не оставл€€ ни кусочка!</span>")
			else if(fullness > 50 && fullness < 150)
				to_chat(M, "<span class='notice'>¬ы прин€лись жадно есть [src].</span>")
			else if(fullness > 150 && fullness < 500)
				to_chat(M, "<span class='notice'>¬ы едите [src].</span>")
			else if(fullness > 500 && fullness < 600)
				to_chat(M, "<span class='notice'>¬ы неохотно кусаете немного [src].</span>")
			...
			if(!isbrain(M))		//If you're feeding it to someone else.
				if(fullness <= (600 * (1 + M.overeatduration / 1000)))
					M.visible_message("<span class='danger'>[user] пытаетс€ накормить [M] [src].</span>", \
										"<span class='userdanger'>[user] пытаетс€ накормить [M] [src].</span>")
				else
