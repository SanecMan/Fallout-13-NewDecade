...
/mob/living/proc/mob_sleep()
	...
	if(sleeping)
		to_chat(src, "<span class='notice'>¬ы уже спите.</span>")
		return
	else
		if(alert(src, "¬ы уверены что хотите немного поспать?", "Sleep", "Yes", "No") == "Yes")
...
/mob/living/proc/lay_down()
	...
	if(!under_object)
		resting = !resting
		to_chat(src, "<span class='notice'>“еперь вы [resting ? "лежите" : "встаете"].</span>")
		update_canmove()
	else
		...
		to_chat(src, "<span class='danger'> огда вы попытались встать, вы ударились о [under_object] своей головой!<br>јй!</span>")
...
/mob/living/proc/surrender()
	...
	if(canmove)
		...
		Stun(5)
		visible_message("<b>[src] сдаЄтс€!</b>")
...
/mob/living/proc/Examine_OOC()
	...
	if(config.allow_Metadata)
		if(client)
			to_chat(src, "[src]'s ћетаинформаци€:<br>[client.prefs.metadata]")
		else
...
/mob/living/verb/resist()
	...
	if(!restrained(ignore_grab = 1) && pulledby)
		visible_message("<span class='danger'>[src] пытаетс€ вырватьс€ из хватки [pulledby]!</span>")
		resist_grab()
		return
...
/mob/living/resist_grab(moving_resist)
	...
	if(pulledby.grab_state)
		if(prob(30/pulledby.grab_state))
			visible_message("<span class='danger'>[src] вырываетс€ из хватки [pulledby]!</span>")
			pulledby.stop_pulling()
			return 0
...
/mob/living/stripPanelUnequip(obj/item/what, mob/who, where)
	...
	if(!(user.perks.have(/datum/perk/thief)))
		who.visible_message("<span class='danger'>[src] пытаетс€ сн€ть [what.name] c [who].</span>", \
			"<span class='userdanger'>[src] пытаетс€ сн€ть [what.name] c [who].</span>")
...
/mob/living/stripPanelEquip(obj/item/what, mob/who, where)
	...
	if(what && (what.flags & NODROP))
		to_chat(src, "<span class='warning'>¬ы не можете поместить [what.name] на [who], это застр€ло у вас в руках!</span>")
		return
	if(what)
		var/list/where_list
		if(islist(where))
			...
			if(!what.mob_can_equip(who, src, where[1], 1))
				to_chat(src, "<span class='warning'>[what.name] не помещаетс€ сюда!</span>")
				return
		else
			if(!what.mob_can_equip(who, src, where, 1))
				to_chat(src, "<span class='warning'>[what.name] не помещаетс€ сюда!</span>")
				return
		visible_message("<span class='notice'>[src] пытаетс€ надеть [what] на [who].</span>")
...
/mob/living/canUseTopic(atom/movable/M, be_close = 0, no_dextery = 0)
	...
	if(no_dextery)
		...
		to_chat(src, "<span class='warning'>” вас недостаточно мозгов чтобы сделать это!</span>")
	return
...
/mob/living/carbon/human/update_stamina()
	if(staminaloss)
		...
		if(total_health <= HEALTH_THRESHOLD_CRIT && !stat)
			to_chat(src, "<span class='notice'>¬ы слишком устали чтобы продолжать...</span>")
			Weaken(5)
...
/mob/living/proc/IgniteMob()
	if(fire_stacks > 0 && !on_fire)
		...
		src.visible_message("<span class='warning'>[src] горит!</span>", \
						"<span class='userdanger'>¬ы горите!</span>")
		set_light(3)
...
/mob/living/proc/spreadFire(mob/living/L)
	...
	if(on_fire) //Only spread fire stacks if we're on fire
		...
		if(L.IgniteMob())
			log_game("[key_name(src)] врезаетс€ в [key_name(L)] и поджигает")
