...
/obj/structure/burnbarrel/attackby(obj/item/P, mob/user, params)
	if(P.is_hot())
		fire(user)
	else if(istype(P, /obj/item/stack/sheet/mineral/wood))
		var/obj/item/stack/sheet/mineral/wood/W = P
...
/obj/structure/burnbarrel/fire_act(exposed_temperature, exposed_volume)
	fire()

/obj/structure/burnbarrel/Crossed(atom/movable/AM)
	if(fired)
		burn_process()

/obj/structure/burnbarrel/process()
	if(fuel <= 0)
		extinguish()
		return
	burn_process()
...
/obj/structure/burnbarrel/proc/fire(mob/living/user)
	...
	START_PROCESSING(SSobj, src)
	fired = 1
	desc = "A burning burnbarrel... A warm, bright, and hopeful fire source.<br><b>It's hot!</b>"
	if(user)
		user.visible_message("[user] has lit a [src].", "<span class='notice'>You have lit a [src].</span>")
	update_icon()
	burn_process()

/obj/structure/burnbarrel/proc/burn_process()
	var/turf/location = get_turf(src)
	for(var/A in location)
		if(A == src)
			continue
...
/obj/structure/burnbarrel/update_icon()
	if(fired)
		icon_state = "barrel_rust_lit"
	else
		icon_state = "barrel_rust"
		overlays.Cut()
	..()
