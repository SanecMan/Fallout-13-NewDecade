...
/obj/item/weapon/storage/proc/can_be_inserted(obj/item/W, stop_messages = 0, mob/user)
	...
	if(W.w_class > max_w_class)
		if(!stop_messages)
			to_chat(usr, "<span class='warning'>[W] слишком большое дл€ [src]!</span>")
		return 0
	...
	if(usr)
		...
		if(!prevent_warning)
			for(var/mob/M in viewers(usr, null))
				if(M == usr)
					to_chat(usr, "<span class='notice'>¬ы кладЄте [W] [preposition] [src].</span>")
				else if(in_range(M, usr)) //If someone is standing close enough, they can tell what it is...
					M.show_message("<span class='notice'>[usr] кладЄт [W] [preposition] [src].</span>", 1)
